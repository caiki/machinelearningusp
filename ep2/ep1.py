...
class multiresolution:
    ...
    def error(self, imgpairs):
        ...
        v = []
        for (srcimg, targetimg) in imgpairs:
            target = self.operators[0].se.cropborders(targetimg)
            output = self.operators[0].se.cropborders(self.apply(srcimg))
            v.append((target, output))
        return mean_dist(v)

    def error_in_sample(self):
        return self.error(self.operators[0].trainingdata)
